...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([
            ...
            # Add this after html.Div(id='signal-display', className="mb-3"),
            html.Hr(),
            html.H5("📋 Signal Log"),
            html.Div(
                id='signal-log-display',
                style={
                    'height': '300px',
                    'overflow-y': 'auto',
            ...
            dbc.Button("🗑️ Clear Log", id='clear-log-btn', color="outline-secondary", size="sm", className="w-100 mb-3"),
...
def save_gex_to_csv(gex_df: pd.DataFrame, spot_price: float, vix_data: dict, symbol: str) -> list[str]:
    """
    Appends current GEX snapshot to a dated CSV and refreshes a 'latest' CSV.
    Returns the list of file paths written (for git add/commit).
    """
    ...
    df.insert(2, 'spot', float(spot_price))
...
def git_add_commit_push(paths: list[str], message: str | None = None, branch: str = 'main') -> None:
    """
    Runs git add/commit/push for the provided paths.
    Assumes the script is running inside a cloned git repo,
    and that 'origin' remote + auth are already configured.
    """
    if not paths:
        return

    repo_dir = Path(__file__).resolve().parent
    commit_msg = message or f'Update GEX snapshots {datetime.now().isoformat(timespec="seconds")}'
    ...
    if commit_proc.returncode != 0:
        # Probably "nothing to commit" — bail quietly
        return
    # git push
...
@app.callback(
    [Output('dashboard-content', 'children'),
     ...
     Output('signal-log-display', 'children')],
...
def update_dashboard(session_data, n_intervals, update_clicks, symbol, strike_range,
    ...
    if not session_data or not session_data.get('logged_in'):
        return html.Div("Please log in to view the dashboard."), html.Div(""), html.Div("No signals logged yet...", className="text-muted")
    ...
    try:
        ...
        # FIXED: Store targets in global history
        global target_history
        if targets:
            target_history.append(targets)
            target_history = target_history[-10:]  # Keep last 10 target sets
            print(f"✅ Updated target_history with {len(targets)} targets")  # Debug print
            print(target_history)

        # --- Save & Push ---
        written_paths = save_gex_to_csv(gex_df, spot_price, vix_data, symbol)
        try:
            # Optional: gate pushing behind an env var
            if os.getenv('GEX_AUTO_PUSH', '1') == '1':
                git_add_commit_push(
                    written_paths,
                    message=f'{symbol.upper()} GEX snapshot {datetime.now():%Y-%m-%d %H:%M:%S}'
                )
        except Exception as git_err:
            # Don’t break the dashboard if git fails
            print(f'⚠️ Git push failed: {git_err}')
        ...
        return dashboard_content, signal_display, build_signal_log()

    except Exception as e:
        print("⚠ Full exception traceback:")
        traceback.print_exc()
        return html.Div(f"⚠ Error updating dashboard: {e}"), html.Div(""), html.Div("Error occurred", className="text-muted")
...
def log_signals(signals):
    ...
    for signal in signals:
        log_entry = {
            ...
            'target_strike': signal.get('target_strike', None)
        }
...
def build_signal_log():
    """Build the signal log display"""
    if not signal_log:
        return html.Div("No signals logged yet...", className="text-muted")

    log_entries = []
    for entry in reversed(signal_log):  # Show newest first
        # Color coding based on signal type
        if 'BULLISH' in entry['signal_type']:
            color_class = "text-success"
        elif 'BEARISH' in entry['signal_type']:
            color_class = "text-danger"
        elif 'WARNING' in entry['signal_type']:
            color_class = "text-warning"
        else:
            color_class = "text-info"

        target_text = f" → {entry['target_strike']:.0f}" if entry['target_strike'] else ""

        log_entries.append(
            html.Div([
                html.Small(entry['timestamp'], className="text-muted me-2"),
                html.Span(f"{entry['signal_type']}{target_text}", className=f"{color_class} fw-bold me-2"),
                html.Span(f"({entry['confidence']:.0f}%)", className="text-muted me-2"),
                html.Br(),
                html.Small(entry['message'], className="text-light ms-4")
            ], className="mb-2")
        )
    signal_log.clear()
...
@app.callback(
    Output('signal-log-display', 'children', allow_duplicate=True),
    Input('clear-log-btn', 'n_clicks'),
    prevent_initial_call=True
)
def clear_signal_log(n_clicks):
    global signal_log
    signal_log = []
    return html.Div("Log cleared.", className="text-muted")
